...
variable (since this is an auto-generated parameterized report).
```{r declare-cat-var}
...
`dot_plot_base` and `bar_plot_base` functions, see the appendix at the bottom.
```{r}
...
Also display in bar plot format, since that can be more legible for some outcome measures.
...
Let's examine how copartisanship varies by individual PID and state leaning.
...
In `process-data.R`, we impute county information based on the ZIP code in which
the respondent said they currently lived.
...
To facilitate comparison between Dems in Dem counties and Reps in Rep counties,
we'll also create a new column indicating whether a county's leaning is
"co-partisan" with an individual person's party ID. So `county_copartisananship`
for a Democrat living in a Democratic county would be "Co-partisan", but a
Republican living in the same county would have the value "Counter-partisan".
```{r}
...
```{r, pid_and_county_cop}
dot_plot('collapsed_pid', 'county_copartisanship')
```

## By Urbanicity

The above are more granular information about locality, but we also do have information
about the urbanicity of the respondent's area. We'll analyze it here.
